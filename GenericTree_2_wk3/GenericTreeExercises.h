...
static void treeFactory(GenericTree<int>& tree) {
  ...
  // ...
  auto rootprt = tree.getRootPtr();
  if (rootprt) {
    tree.clear();
  }
  
  tree.createRoot(4);
  auto newrootprt = tree.getRootPtr();
  auto newl2leftnode = newrootprt->addChild(8); 
  newrootprt->addChild(15); // rightmost on level 2
  auto newl3leftnode = newl2leftnode->addChild(16); 
  newl2leftnode->addChild(23); // rightmost on level 3
  newl3leftnode->addChild(42); // rightmost on level 4 
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::queue<TreeNode*> NodestoExplored;
  NodestoExplored.push(rootNodePtr);

  results.push_back(rootNodePtr->data); 

  while(!NodestoExplored.empty()){
    TreeNode* frontNode = NodestoExplored.front();
    NodestoExplored.pop();
