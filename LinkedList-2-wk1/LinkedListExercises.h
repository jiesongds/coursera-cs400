...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (head_->data > in_node->data){
    Node* oldhead = head_;
    oldhead->prev = in_node;
    in_node->next = oldhead;
    head_ = in_node;
    size_++;
    return;
  }

  if (tail_->data <= in_node->data){
    Node* oldtail = tail_;
    in_node->prev = oldtail;
    oldtail->next = in_node;
    tail_ = in_node;
    size_++;
    return;
  }
  ...
  Node* cur = head_;
  while (cur->next){
    if (cur->next->data > in_node->data){
      in_node->next = cur->next; 
      in_node->prev = cur->next->prev;
      cur->next->prev = in_node;
      cur->next = in_node;
      size_++;
      return;
    }
